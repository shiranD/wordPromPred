...
def featurize(line, idnum, pat):
    ...
    with open(str(filename), 'a') as file: #open json file
        ...
        for (word,tag) in word_tag:
            ...
            current_word.append(('function','True' if word in function else 'False'))
            current_word.append(('negation','True' if word in negation else 'False'))
            try: 
                phns = d[word.lower()][0]
            except:
                f.write(word)
