...
class Swbdnext(str):
    ...
    def disfluency(self):
        ...
        if first:
            ...
            event = []
            first1 = 0
            for elem in self.disfluency_tree.iter():# extract all terminal refs defaut type "repara"
                    if elem.tag == "{http://nite.sourceforge.net/}child":
                        self.disf_dict[elem.attrib["href"].rstrip(')').split('(')[1]] = "reparandum"
                    elif first1:
                        event.append(elem.attrib["{http://nite.sourceforge.net/}id"])
                    first1 = 1
                    
            for elem in self.disfluency_tree.iter("repair"):# extract all repair and modify dict accordingly
                ...
                if sub_num==4:
                    for child in elem:
                        if child.tag == "{http://nite.sourceforge.net/}child":
                            self.disf_dict[child.attrib["href"].rstrip(')').split('(')[1]] = "repair"
                else: # if is found in sub hirarchy, check if ancestors are repair as well
                    for itr in xrange(sub_num):  
        ...
        try: # check termianl type
            self.disf_dict[self.word_id]
            #print self.word_id
            self.current_word.append(("disf_stat",self.disf_dict[self.word_id]))
        except:
