...
weight = np.zeros((num_clas, l_X * (2 * N + 1) + l_label), dtype=float)
weight_sum = np.zeros((num_clas, l_X * (2 * N + 1) + l_label), dtype=float)
...
for it in xrange(num_iter):
    ...
    for g, (sample, label) in enumerate(zip(X_train, Y_train)):
        ...
        APsample = np.append(last[-(l_X * N + 3):], fut_sample)
        ...
        last = np.append(last[-l_X * N:], greed)  # update last
